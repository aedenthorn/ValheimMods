...
namespace AutoFuel
{
	...
	public class BepInExPlugin : BaseUnityPlugin
	{
		...
		public static ConfigEntry<bool> refuelFromInventory;
		public static ConfigEntry<float> inventoryRange;
		...
		{
			context = this;
			refuelFromInventory = Config.Bind<bool>("General", "RefuelFromInventory", false, "Refuel from player inventory");
			inventoryRange = Config.Bind<float>("General", "InventoryRange", 5f, "The maximum range to pull fuel from the player inventory (if enabled)");
			...
			try
			{
				...
				foreach (Container c in nearbyContainers)
				{
					if (fireplace.m_fuelItem && maxFuel > 0)
					{
						...
						{
							var fill = TryFillFireWithItem(fireplace, znview, fuelItem, ref maxFuel, container: c);
							if (fill && distributedFilling.Value) return;
						}
					}
				}

				if (refuelFromInventory.Value && maxFuel > 0)
				{
					var distance = Vector3.Distance(fireplace.transform.position, Player.m_localPlayer.transform.position);
					if (distance <= inventoryRange.Value)
					{
						List<ItemDrop.ItemData> itemList = new List<ItemDrop.ItemData>();
						var inventory = Player.m_localPlayer.GetInventory();
						inventory.GetAllItems(fireplace.m_fuelItem.m_itemData.m_shared.m_name, itemList);
						foreach (var fuelItem in itemList)
						{
							var fill = TryFillFireWithItem(fireplace, znview, fuelItem, ref maxFuel, inventory: inventory);
							if (fill && distributedFilling.Value) return;
						}
					}
				}
		...
		static bool TryFillFireWithItem(Fireplace fireplace, ZNetView znview, ItemDrop.ItemData fuelItem, ref int maxFuel, Container container = null, Inventory inventory = null)
		{
			if (inventory == null) inventory = container.GetInventory();
			...
			if (fuelItem != null && (!leaveLastItem.Value || fuelItem.m_stack > 1))
			{
				...
				{
					// Dbgl($"container at {container?.transform?.position} has {fuelItem.m_stack} {fuelItem.m_dropPrefab.name} but it's forbidden by config");
					return false;
				}
				znview.InvokeRPC("AddFuel", new object[] { });
				inventory.RemoveItem(fireplace.m_fuelItem.m_itemData.m_shared.m_name, 1);
				if (container != null) typeof(Container).GetMethod("Save", BindingFlags.NonPublic | BindingFlags.Instance).Invoke(container, new object[] { });
